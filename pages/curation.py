    ...
    if selected_chromosomes:
        ...
        if len(selected_chromosomes) == 1:
            ...
            if os.path.exists(pdf_path):
                ...
                st.image(chrom_pdf_image, caption=f"Chromosome {extract_chromosome_number(selected_chromosomes[0])}", use_column_width=True)
            ...
            for col, pdf_file in zip(display_cols, selected_chromosomes):
                ...
                if os.path.exists(pdf_path):
                    chrom_pdf_image = get_pdf_first_page_image(pdf_path)
                    col.image(chrom_pdf_image)#, caption=f"Chromosome {extract_chromosome_number(pdf_file)[0]}", use_column_width=True)    
                else:
